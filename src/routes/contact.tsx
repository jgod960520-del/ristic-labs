...
export default function Contact() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const email =
      formData.get("email")?.toString().trim() || "No email provided";
    const company =
      formData.get("company")?.toString().trim() || "Not provided";
    const message =
    ...
    if (typeof window !== "undefined") {
      window.location.href = `mailto:contact@risticinnovations.technology?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    }
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Get in touch with Ristic Labs, the blockchain innovation division based in Wilmington, Delaware."
        />
        <meta property="og:title" content="Contact Ristic Labs" />
        <meta
          property="og:description"
          content="Let's build something on-chain together."
        />
      </Head>
      <SiteShell>
        <section className="mx-auto grid max-w-7xl gap-16 px-6 pb-24 pt-20 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-6 max-w-md text-lg text-muted-foreground">
              Whether you're refining a protocol idea or scaling a production
              network — we'd love to hear from you.
            </p>
            <div className="mt-10 space-y-5">
              <div className="flex items-start gap-4">
                <span className="mt-1 grid h-10 w-10 place-items-center rounded-lg bg-primary/10 text-primary">
                  <Mail size={18} />
                </span>
                <div>
                  ...
                  <div className="mt-1 font-medium">
                    contact@risticinnovations.technology
                  </div>
                </div>
              </div>
              <div className="flex items-start gap-4">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <a
                    href="https://risticinnovations.technology"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-1 font-medium hover:text-primary"
                  >
                    risticinnovations.technology
                  </a>
                </div>
              </div>
            </div>
